...
import {
 ...
 overwriteFile, saveSolidDatasetAt,
 setThing, setUrl
...
//handling them files
async function uploadMetadata(metadataUrl:string, metadata:Metadata) {
 // Create a new Thing for the metadata
 const metadataThing = createThing();

 // Add the metadata properties to the Thing
 const updatedMetadataThing = setUrl(setUrl(setUrl(setUrl(metadataThing,
         FOAF.title, metadata.title),
       FOAF.maker, metadata.author),
     FOAF.depicts, metadata.tagged.join(',')),
   FOAF.depiction, metadata.description);

 // Create an empty RDF file
 let dataset = await getSolidDataset(metadataUrl, { fetch });

 // Add the metadata Thing to the dataset
 dataset = setThing(dataset, updatedMetadataThing);
...
async function handleFiles() {
 ...
 const imageTargetUrl = `${webId}/image${selectedFile.value}.jpg`;
 const metadataTargetUrl = `${webId}/metadata${selectedFile.value}.ttl`;
 ...
 await uploadMetadata(metadataTargetUrl, metadata);
